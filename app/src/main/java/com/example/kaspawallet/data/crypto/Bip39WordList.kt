...
object Bip39WordList {
    val WORDS: List<String> = listOf(
        ...
        "office", "officer", "offline", "often", "oil", "okay", "old", "olive", "olympic", "omit",
        ...
        "use", "used", "useful", "useless", "usual", "utility", "vacant", "vacuum", "vague", "valuable",
    ...
    fun generateMnemonic(wordCount: Int = 12): List<String> {
        val count = if (wordCount == 24) 24 else 12
        val entropyBytes = if (count == 24) 32 else 16
    ...
    fun entropyToMnemonic(entropy: ByteArray): List<String> {
        ...
        val totalBits = entBits + csBits
        val bits = BooleanArray(totalBits)

        for (i in 0 until entBits) {
            val byteIdx = i / 8
            val bitIdx = 7 - (i % 8)
            bits[i] = ((entropy[byteIdx].toInt() shr bitIdx) and 1) == 1
        }
        ...
        val result = ArrayList<String>(wordCount)
    ...
    fun validateMnemonic(words: List<String>): Boolean {
        ...
        for (i in words.indices) {
            ...
            for (b in 0 until 11) {
                val bitPos = 10 - b
                bits[i * 11 + b] = ((wordIdx shr bitPos) and 1) == 1
            }
        }
    ...
    fun isValidWord(word: String): Boolean {
        return WORDS.contains(word.trim().lowercase())
    }

    fun suggestWords(prefix: String, limit: Int = 5): List<String> {
        if (prefix.isBlank()) return emptyList()
        val clean = prefix.trim().lowercase()
        return WORDS.filter { it.startsWith(clean) }.take(limit)
    }
}
